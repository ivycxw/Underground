...
public class CameraOrbit : MonoBehaviour
{
	...
	[SerializeField] private bool MiddleMouseOrbit = true;
	[SerializeField] private bool Mouse = true;
	[SerializeField] private bool ShowMouse = true;
	[SerializeField] private bool Showcase = true;
	[SerializeField] private bool Touch = false;
	...
	[SerializeField] private float maxFov = 35.0f;
	...
	private float frontalRotation = 15f;
	...
	private void Start()
	{
		...
		if (maxFov < Camera.main.fieldOfView) maxFov = Camera.main.fieldOfView + 10;
		if (rotationTarget != null) centerOffset = rotationTarget.position + centerOffset;
	}
	...
	private void LateUpdate()
	{
		...
		{	
			...
			{
				float tZoom = Input.GetAxis("Mouse ScrollWheel") * scrollSpeed;
				if (Camera.main.fieldOfView + tZoom < maxFov) Camera.main.fieldOfView += tZoom; //+
			}

			if (Input.GetKey(KeyCode.DownArrow) || Input.GetAxis("Mouse ScrollWheel") < 0)
			{
				float tZoom = Input.GetAxis("Mouse ScrollWheel") * scrollSpeed;
				if (Camera.main.fieldOfView + tZoom > minFov) Camera.main.fieldOfView += tZoom; //-
			}
		}

		if (Mouse)
		{
			if ((Input.GetMouseButton(1) && !MiddleMouseOrbit) || (Input.GetMouseButton(2) && MiddleMouseOrbit))
			{
				easeOut = false;
				fadeTimer = 1.0f;
			...
			if ((Input.GetMouseButtonUp(1) && !MiddleMouseOrbit) && (ScaledDeltaSpeed.x > 2f || ScaledDeltaSpeed.x < -2f) ||
			    (Input.GetMouseButtonUp(2) && MiddleMouseOrbit) && (ScaledDeltaSpeed.x > 2f || ScaledDeltaSpeed.x < -2f))
			{
				easeOut = true;
		...
		if (Input.touchCount == 1 || Input.GetMouseButton(1) && !MiddleMouseOrbit ||
		    Input.GetMouseButton(2) && MiddleMouseOrbit)
		{
			if (float.IsNaN(ScaledDeltaSpeed.x) || float.IsNaN(ScaledDeltaSpeed.y) || float.IsInfinity(ScaledDeltaSpeed.x) ||
			    float.IsInfinity(ScaledDeltaSpeed.y)) ScaledDeltaSpeed = new Vector2(0, 0);
	...
	private void RotateAround(Vector3 axis, float direction)
	{
		float angle;
		if (axis == t.right)
		{
		...
		t.RotateAround(centerOffset, axis, angle);
	}
	...
	private void EaseOutRotation(float ScaledDeltaSpeedX)
	{
	...
	/// </summary>
	public void ShowcaseEnabled(bool Enabled)
	{
		Showcase = Enabled;
	}
}